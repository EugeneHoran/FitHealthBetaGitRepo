...
public class LogRecipeItems extends Model {
    protected ForeignKeyField<LogRecipeHolder> dbForeignKey;
    ...
    public LogRecipeItems() {
        super(true);
        dbForeignKey = new ForeignKeyField<LogRecipeHolder>(LogRecipeHolder.class);
    ...
    public void setForeignKey(LogRecipeHolder meal) {
        dbForeignKey.set(meal);
    }

    public LogRecipeHolder getForeignKey(Context context) {
        return dbForeignKey.get(context);
    }
    ...
    public static List<LogRecipeItems> logSortByMealChoice(String b, Date d) {
        ...
        return LogRecipeItems.objects().filter(filter).orderBy("-date").toList();
    }


    public static List<LogRecipeItems> logSortByManual(String b) {
        String query_meal = formatProjectForQuery(b);
        Filter filter = new Filter();
        filter.is("manualEntry", query_meal);
        return LogRecipeItems.objects().filter(filter).toList();
    }

    public static List<LogRecipeItems> logSortByFavorite(String b) {
        Filter filter = new Filter();
        filter.is("mFavorite", b);
        return LogRecipeItems.objects().filter(filter).toList();
    }

    public static List<LogRecipeItems> logSortByFavoriteMeal(String b, String meal) {
        Filter filter = new Filter();
        filter.is("mFavorite", b);
    ...
    public static List<LogRecipeItems> logsById(int date) {
        String hello = String.valueOf(date);
        Filter filter = new Filter();
        filter.contains("mealId", hello);
        return LogRecipeItems.objects().filter(filter).orderBy("mId").toList();
    }
     ...
     * @return
     */
    public static List<LogRecipeItems> logsByDate(Date date) {
    ...
    public static List<LogRecipeItems> logsByDateSAndMealType(Date date, String mealType) {
        String query_string = formatDateForQuery(date);
        Filter filter = new Filter();
        filter.contains("date", query_string);
     ...
     * @return
     */
    public static List<LogRecipeItems> all() {
        return LogRecipeItems.objects().all().orderBy("date").toList();
    }
     ...
     * @return
     */
    public static QuerySet<LogRecipeItems> objects() {
        return LogRecipeItems.objects(context(), LogRecipeItems.class);
    }
